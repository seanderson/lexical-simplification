...
def wordStats(probs,indices,fout=""):
  ...
  for i,p in enumerate(probs): # all words (each an array)
      num += 1
      maxidx = np.argmax(p[:-1]) #omit last column, which is prob of actual word
      minidx = np.argmin(p[:-1])
      maxval = p[maxidx]
      idx = indices[i][maxidx]
      iscorrect = 0
      correct_output = indices[i][-1]
      if idx == correct_output:
        iscorrect = 1
        numcorrect += 1
      msglst = (iscorrect, maxval/p[minidx],idx,correct_output,p[-1])
      if writeOutput:
