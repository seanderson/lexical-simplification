...
class FeatureEstimator:
            ...
            for subst in line[3:len(line)]:
                ...
                for word in words.split(' '):
                    ...
                    try:
                        ...
                        try:
                            similarities.append(
                                model.similarity(target, word.lower()))
                        except KeyError:
                            pass
    ...
    def collocationalFeature(self, data, args):
        ...
        for line in data:
            ...
            for subst in line[3:len(line)]:
                ...
                for span1 in spanlv:
                    for span2 in spanrv:
                        ngram, bosv, eosv = self.getNgram(word, sent, head,
                                                          span1, span2)
    ...
    def taggedFrequencyCollocationalFeature(self, data, args):
        ...
        for i in range(0, len(data)):
            line = data[i]
            sent = ['<s>'] + [tokendata[1] for tokendata in tagged_sents[i]] + [
                '</s>']
            target = line[1]
            head = int(line[2]) + 1
            spanlv = range(0, spanl + 1)
    ...
    def popCollocationalFeature(self, data, args):
        ...
        for line in data:
            ...
            for subst in line[3:len(line)]:
                ...
                for span1 in spanlv:
                    for span2 in spanrv:
                        ngrams = self.getPopNgrams(word, sent, head, span1,
                                                   span2)
                        maxscore = -999999
                        for ngram in ngrams:
                            aux = model.score(ngram[0], bos=ngram[1],
                                              eos=ngram[2])
    ...
    def ngramProbabilityFeature(self, data, args):
        ...
        for line in data:
            ...
            for subst in line[3:len(line)]:
                word = subst.split(':')[1].strip()
                ngram, bosv, eosv = self.getNgram(word, sent, head, spanl,
                                                  spanr)
        ...
        if configl == 0 and configr == 0:
            ...
            for context in contexts:
                ...
                for i in range(max(0, chead - configl), chead):
                    ngram += ctokens[i] + ' '
                ngram += cand + ' '
                for i in range(chead + 1,
                               min(len(ctokens), chead + configr + 1)):
                    ngram += ctokens[i] + ' '
    ...
    def reverseSentenceProbabilityFeature(self, data, args):
        ...
        for line in data:
            ...
            for subst in line[3:len(line)]:
                word = subst.split(':')[1].strip()
                ngram, bosv, eosv = self.getNgram(word, invsent, head, 9999,
                                                  9999)
                aux = model.score(ngram, bos=bosv, eos=eosv)
                result.append(aux)
        return result
            ...
            for subst in line[3:len(line)]:
                ...
                for word in words.split(' '):
                    try:
                        ...
                        senses = senses
                if len(tgtsenses) == 0 or len(
                        senses.intersection(tgtsenses)) > 0:
                    resultsy.append(1.0)
                else:
                    resultsy.append(0.0)
        return resultsy
            ...
            for subst in line[3:len(line)]:
                ...
                for word in words.split(' '):
                    try:
                        ...
                        senses = senses
                if len(tgthypernyms) == 0 or len(
                        senses.intersection(tgthypernyms)) > 0:
                    resultsy.append(1.0)
                else:
                    resultsy.append(0.0)
        return resultsy
            ...
            for subst in line[3:len(line)]:
                ...
                for word in words.split(' '):
                    try:
                        ...
                        senses = senses
                if len(tgthyponyms) == 0 or len(
                        senses.intersection(tgthyponyms)) > 0:
                    resultsy.append(1.0)
                else:
                    resultsy.append(0.0)
        return resultsy
            ...
            for subst in line[3:len(line)]:
                ...
                if len(insts) > 0:
                    for inst in insts:
                        ...
                        prob = math.exp(
                            model.score(ngram, bos=False, eos=False))
        ...
        if 'inv_dep_maps' in self.temp_resources:
            ...
            if 'dep_maps' in self.temp_resources:
                ...
                if 'dep_parsed_sents' in self.temp_resources:
                    ...
                    dep_parsed_sents = dependencyParseSentences(parser,
                                                                sentences)
                    self.temp_resources['dep_parsed_sents'] = dep_parsed_sents
                dep_maps = []
                for sent in dep_parsed_sents:
                    ...
                    for parse in sent:
                        ...
                        objectindex = int(str(parse[4])) - 1
                        if subjectindex not in dep_map:
                            dep_map[subjectindex] = {
                                objectindex: set([deplink])}
                        elif objectindex not in dep_map[subjectindex]:
                            dep_map[subjectindex][objectindex] = set([deplink])
                        else:
                            dep_map[subjectindex][objectindex].add(deplink)
            ...
            for inst in dep_maps:
                ...
                for subjectindex in inst:
                    for objectindex in inst[subjectindex]:
                        ...
                        inv_dep_map[objectindex][subjectindex] = \
                        inst[subjectindex][objectindex]
            ...
            for subst in line[3:len(line)]:
                ...
                for content_word in content_words:
                    try:
                        ...
                        try:
                            similarity += model.similarity(content_word,
                                                           word.lower())
                        except KeyError:
                            pass
                similarity /= divisor
    ...
    def backoffBehaviorNominalFeature(self, data, args):
        ...
        for line in data:
            ...
            for subst in line[3:len(line)]:
                ...
                if head > 0:
                    ngram2f, bos2f, eos2f = self.getNgram(sent[head - 1], sent,
                                                          head - 1, 1, 0)
                    ngram1f, bos1f, eos1f = self.getNgram(sent[head - 1], sent,
                                                          head - 1, 0, 0)
    ...
    def POSNgramNominalFeature(self, data, args):
        ...
        for i in range(0, len(data)):
            ...
            for subst in line[3:len(line)]:
                ngram, bosv, eosv = self.getNgram(target_pos, POStokens, head,
                                                  spanl, spanr)
                tokens = ngram.split(' ')
                fngram = ''
                for token in tokens:
    ...
    def POSNgramWithCandidateNominalFeature(self, data, args):
        ...
        for i in range(0, len(data)):
            ...
            for subst in line[3:len(line)]:
                word = subst.split(':')[1].strip()
                ngram, bosv, eosv = self.getNgram(word, POStokens, head, spanl,
                                                  spanr)
                tokens = ngram.split(' ')
                fngram = ''
                for token in tokens:
    ...
    def getPageCount(self, word):
        ...
        try:
            ...
            count = int(
                result[0].strip().split(' ')[0].strip().replace(',', ''))
        except Exception:
            count = 0
        return count
    ...
    def isContentWord(self, word, tag):
        content_tags = set(
            ['JJ', 'JJS', 'JJR', 'NN', 'NNS', 'RB', 'RBR', 'RBS', 'VB', 'VBD',
             'VBG', 'VBN', 'VBP', 'VBZ'])
    ...
    def addWordVectorValues(self, model, size, orientation):
        ...
        if orientation not in ['Complexity', 'Simplicity']:
            ...
            for i in range(0, size):
                self.identifiers.append(('Word Vector Value ' + str(
                    i) + ' (Model: ' + model + ')', orientation))

    def addTargetPOSTagProbability(self, condprob_model, pos_model,
                                   stanford_tagger, java_path, orientation):
        """
        ...
        if orientation not in ['Complexity', 'Simplicity']:
            ...
            self.features.append(
                (self.targetPOSTagProbability, [condprob_model, pos_model]))
            self.identifiers.append(('Target POS Tag Probability (Model:' + str(
                condprob_model) + ')', orientation))
    ...
    def addWordVectorSimilarityFeature(self, model, orientation):
        ...
        if orientation not in ['Complexity', 'Simplicity']:
            ...
            self.identifiers.append(
                ('Word Vector Similarity (Model: ' + model + ')', orientation))

    def addTaggedWordVectorSimilarityFeature(self, model, pos_model,
                                             stanford_tagger, java_path,
                                             pos_type, orientation):
        """
        ...
        if orientation not in ['Complexity', 'Simplicity']:
            ...
            if pos_model not in self.resources:
                tagger = StanfordPOSTagger(pos_model, stanford_tagger)
                self.resources[pos_model] = tagger
            self.features.append((self.taggedWordVectorSimilarityFeature,
                                  [model, pos_model, pos_type]))
            self.identifiers.append((
                                    'Word Vector Similarity (Model: ' + model + ') (POS Model: ' + pos_model + ') (POS Type: ' + pos_type + ')',
                                    orientation))

    def addTranslationProbabilityFeature(self, translation_probabilities,
                                         orientation):
        """
        ...
        if orientation not in ['Complexity', 'Simplicity']:
            ...
            self.features.append((self.translationProbabilityFeature,
                                  [translation_probabilities]))
            self.identifiers.append((
                                    'Translation Probability (File: ' + translation_probabilities + ')',
                                    orientation))
    ...
    def addLexiconFeature(self, lexicon, orientation):
        ...
        if orientation not in ['Complexity', 'Simplicity']:
            ...
            self.identifiers.append(
                ('Lexicon Occurrence (Lexicon: ' + lexicon + ')', orientation))
    ...
    def addCollocationalFeature(self, language_model, leftw, rightw,
                                orientation):
        """
        ...
        if orientation not in ['Complexity', 'Simplicity']:
            ...
            self.features.append(
                (self.collocationalFeature, [language_model, leftw, rightw]))
            for i in range(0, leftw + 1):
                for j in range(0, rightw + 1):
                    self.identifiers.append(('Collocational Feature [' + str(
                        i) + ', ' + str(j) + '] (LM: ' + language_model + ')',
                                             orientation))

    def addFrequencyCollocationalFeature(self, ngram_file, leftw, rightw,
                                         orientation):
        """
        ...
        if orientation not in ['Complexity', 'Simplicity']:
            ...
            self.features.append((self.frequencyCollocationalFeature,
                                  [ngram_file, leftw, rightw]))
            for i in range(0, leftw + 1):
                for j in range(0, rightw + 1):
                    self.identifiers.append((
                                            'Frequency Collocational Feature [' + str(
                                                i) + ', ' + str(
                                                j) + '] (N-Grams File: ' + ngram_file + ')',
                                            orientation))

    def addTaggedFrequencyCollocationalFeature(self, ngram_file, leftw, rightw,
        ...
        if orientation not in ['Complexity', 'Simplicity']:
            ...
            if pos_model not in self.resources:
                tagger = StanfordPOSTagger(pos_model, stanford_tagger)
                self.resources[pos_model] = tagger
            self.features.append((self.taggedFrequencyCollocationalFeature,
                                  [ngram_file, leftw, rightw, pos_model,
                                   pos_type]))
            for i in range(0, leftw + 1):
                for j in range(0, rightw + 1):
                    self.identifiers.append((
                                            'Tagged Frequency Collocational Feature [' + str(
                                                i) + ', ' + str(
                                                j) + '] (N-Grams File: ' + ngram_file + ') (POS type: ' + pos_type + ')',
                                            orientation))

    def addBinaryTaggedFrequencyCollocationalFeature(self, ngram_file, leftw,
                                                     rightw, pos_model,
                                                     stanford_tagger, java_path,
                                                     pos_type, orientation):
        """
        ...
        if orientation not in ['Complexity', 'Simplicity']:
            ...
            if pos_model not in self.resources:
                ...
                self.resources[pos_model] = tagger
            self.features.append((
                                 self.binaryTaggedFrequencyCollocationalFeature,
                                 [ngram_file, leftw, rightw, pos_model,
                                  pos_type]))
            for i in range(0, leftw + 1):
                for j in range(0, rightw + 1):
                    self.identifiers.append((
                                            'Binary Tagged Frequency Collocational Feature [' + str(
                                                i) + ', ' + str(
                                                j) + '] (N-Grams File: ' + ngram_file + ') (POS type: ' + pos_type + ')',
                                            orientation))

    def addPopCollocationalFeature(self, language_model, leftw, rightw,
                                   orientation):
        """
        ...
        if orientation not in ['Complexity', 'Simplicity']:
            ...
            if language_model not in self.resources:
                ...
                self.resources[language_model] = model
            self.features.append(
                (self.popCollocationalFeature, [language_model, leftw, rightw]))
            for i in range(0, leftw + 1):
                for j in range(0, rightw + 1):
                    self.identifiers.append((
                                            'Pop Collocational Feature [' + str(
                                                i) + ', ' + str(
                                                j) + '] (LM: ' + language_model + ')',
                                            orientation))

    def addNGramProbabilityFeature(self, language_model, leftw, rightw,
                                   orientation):
        """
        ...
        if orientation not in ['Complexity', 'Simplicity']:
            ...
            if language_model not in self.resources:
                ...
                self.resources[language_model] = model
            self.features.append(
                (self.ngramProbabilityFeature, [language_model, leftw, rightw]))
            self.identifiers.append(('N-Gram Probability Feature [' + str(
                leftw) + ', ' + str(rightw) + '] (LM: ' + language_model + ')',
                                     orientation))
    ...
    def addNGramFrequencyFeature(self, ngram_file, leftw, rightw, orientation):
        ...
        if orientation not in ['Complexity', 'Simplicity']:
            ...
            if ngram_file not in self.resources:
                ...
                self.resources[ngram_file] = counts
            self.features.append(
                (self.ngramFrequencyFeature, [ngram_file, leftw, rightw]))
            self.identifiers.append(('N-Gram Frequency Feature [' + str(
                leftw) + ', ' + str(
                rightw) + '] (N-grams File: ' + ngram_file + ')', orientation))

    def addBinaryNGramFrequencyFeature(self, ngram_file, leftw, rightw,
                                       orientation):
        """
        ...
        if orientation not in ['Complexity', 'Simplicity']:
            ...
            if ngram_file not in self.resources:
                ...
                self.resources[ngram_file] = counts
            self.features.append(
                (self.binaryNgramFrequencyFeature, [ngram_file, leftw, rightw]))
            self.identifiers.append((
                                    'Binary N-Gram Probability Feature [' + str(
                                        leftw) + ', ' + str(
                                        rightw) + '] (N-grams File: ' + ngram_file + ')',
                                    orientation))

    def addPopNGramProbabilityFeature(self, language_model, leftw, rightw,
                                      orientation):
        """
        ...
        if orientation not in ['Complexity', 'Simplicity']:
            ...
            if language_model not in self.resources:
                model = kenlm.LanguageModel(language_model)
                self.resources[language_model] = model
            self.features.append((self.popNgramProbabilityFeature,
                                  [language_model, leftw, rightw]))
            self.identifiers.append(('Pop N-Gram Frequency Feature [' + str(
                leftw) + ', ' + str(rightw) + '] (LM: ' + language_model + ')',
                                     orientation))

    def addPopNGramFrequencyFeature(self, ngram_file, leftw, rightw,
                                    orientation):
        """
        ...
        if orientation not in ['Complexity', 'Simplicity']:
            ...
            if ngram_file not in self.resources:
                ...
                self.resources[ngram_file] = counts
            self.features.append(
                (self.popNgramFrequencyFeature, [ngram_file, leftw, rightw]))
            self.identifiers.append(('Pop N-Gram Frequency Feature [' + str(
                leftw) + ', ' + str(
                rightw) + '] (N-grams File: ' + ngram_file + ')', orientation))
    ...
    def addSentenceProbabilityFeature(self, language_model, orientation):
        ...
        if orientation not in ['Complexity', 'Simplicity']:
            ...
            if language_model not in self.resources:
                ...
                self.resources[language_model] = model
            self.features.append(
                (self.sentenceProbabilityFeature, [language_model]))
            self.identifiers.append((
                                    'Sentence Probability (LM: ' + language_model + ')',
                                    orientation))
    ...
    def addReverseSentenceProbabilityFeature(self, language_model, orientation):
        ...
        if orientation not in ['Complexity', 'Simplicity']:
            ...
            if language_model not in self.resources:
                ...
                self.resources[language_model] = model
            self.features.append(
                (self.reverseSentenceProbabilityFeature, [language_model]))
            self.identifiers.append((
                                    'Reverse Sentence Probability (LM: ' + language_model + ')',
                                    orientation))
    ...
    def addPrefixProbabilityFeature(self, language_model, orientation):
        ...
        if orientation not in ['Complexity', 'Simplicity']:
            ...
            if language_model not in self.resources:
                ...
                self.resources[language_model] = model
            self.features.append(
                (self.prefixProbabilityFeature, [language_model]))
            self.identifiers.append((
                                    'Prefix Probability (LM: ' + language_model + ')',
                                    orientation))
    ...
    def addReversePrefixProbabilityFeature(self, language_model, orientation):
        ...
        if orientation not in ['Complexity', 'Simplicity']:
            ...
            if language_model not in self.resources:
                ...
                self.resources[language_model] = model
            self.features.append(
                (self.reversePrefixProbabilityFeature, [language_model]))
            self.identifiers.append((
                                    'Reverse Prefix Probability (LM: ' + language_model + ')',
                                    orientation))
    ...
    def addSubjectDependencyProbabilityFeature(self, language_model,
                                               stanford_parser,
                                               dependency_models, java_path,
                                               orientation):
        """
        ...
        if orientation not in ['Complexity', 'Simplicity']:
            ...
            if dependency_models not in self.resources:
                parser = StanfordParser(path_to_jar=stanford_parser,
                                        path_to_models_jar=dependency_models)
                self.resources[dependency_models] = parser
            self.features.append((self.subjectDependencyProbabilityFeature,
                                  [language_model, dependency_models]))
            self.identifiers.append((
                                    'Subject Dependency Probability Feature (Language Model: ' + language_model + ') (Models: ' + dependency_models + ')',
                                    orientation))

    def addBinarySubjectDependencyFeature(self, dep_counts_file,
                                          stanford_parser, dependency_models,
                                          java_path, orientation):
        """
        ...
        if orientation not in ['Complexity', 'Simplicity']:
            ...
            if dependency_models not in self.resources:
                parser = StanfordParser(path_to_jar=stanford_parser,
                                        path_to_models_jar=dependency_models)
                self.resources[dependency_models] = parser
            self.features.append((self.binarySubjectDependencyFeature,
                                  [dep_counts_file, dependency_models]))
            self.identifiers.append((
                                    'Binary Subject Dependency Feature (Dependency Link Counts File: ' + dep_counts_file + ') (Models: ' + dependency_models + ')',
                                    orientation))

    def addSubjectDependencyFrequencyFeature(self, dep_counts_file,
                                             stanford_parser, dependency_models,
                                             java_path, orientation):
        """
        ...
        if orientation not in ['Complexity', 'Simplicity']:
            ...
            if dependency_models not in self.resources:
                parser = StanfordParser(path_to_jar=stanford_parser,
                                        path_to_models_jar=dependency_models)
                self.resources[dependency_models] = parser
            self.features.append((self.subjectDependencyFrequencyFeature,
                                  [dep_counts_file, dependency_models]))
            self.identifiers.append((
                                    'Subject Dependency Frequency Feature (Dependency Link Counts File: ' + dep_counts_file + ') (Models: ' + dependency_models + ')',
                                    orientation))

    def addObjectDependencyProbabilityFeature(self, language_model,
                                              stanford_parser,
                                              dependency_models, java_path,
                                              orientation):
        """
        ...
        if orientation not in ['Complexity', 'Simplicity']:
            ...
            if dependency_models not in self.resources:
                parser = StanfordParser(path_to_jar=stanford_parser,
                                        path_to_models_jar=dependency_models)
                self.resources[dependency_models] = parser
            self.features.append((self.objectDependencyProbabilityFeature,
                                  [language_model, dependency_models]))
            self.identifiers.append((
                                    'Object Dependency Probability Feature (Language Model: ' + language_model + ') (Models: ' + dependency_models + ')',
                                    orientation))

    def addBinaryObjectDependencyFeature(self, dep_counts_file, stanford_parser,
                                         dependency_models, java_path,
                                         orientation):
        """
        ...
        if orientation not in ['Complexity', 'Simplicity']:
            ...
            if dependency_models not in self.resources:
                parser = StanfordParser(path_to_jar=stanford_parser,
                                        path_to_models_jar=dependency_models)
                self.resources[dependency_models] = parser
            self.features.append((self.binaryObjectDependencyFeature,
                                  [dep_counts_file, dependency_models]))
            self.identifiers.append((
                                    'Binary Object Dependency Feature (Dependency Link Counts File: ' + dep_counts_file + ') (Models: ' + dependency_models + ')',
                                    orientation))

    def addObjectDependencyFrequencyFeature(self, dep_counts_file,
                                            stanford_parser, dependency_models,
                                            java_path, orientation):
        """
        ...
        if orientation not in ['Complexity', 'Simplicity']:
            ...
            if dependency_models not in self.resources:
                parser = StanfordParser(path_to_jar=stanford_parser,
                                        path_to_models_jar=dependency_models)
                self.resources[dependency_models] = parser
            self.features.append((self.objectDependencyFrequencyFeature,
                                  [dep_counts_file, dependency_models]))
            self.identifiers.append((
                                    'Object Dependency Frequency Feature (Dependency Link Counts File: ' + dep_counts_file + ') (Models: ' + dependency_models + ')',
                                    orientation))

    def addAllDependencyProbabilityFeature(self, language_model,
                                           stanford_parser, dependency_models,
                                           java_path, orientation):
        """
        ...
        if orientation not in ['Complexity', 'Simplicity']:
            ...
            if dependency_models not in self.resources:
                parser = StanfordParser(path_to_jar=stanford_parser,
                                        path_to_models_jar=dependency_models)
                self.resources[dependency_models] = parser
            self.features.append((self.allDependencyProbabilityFeature,
                                  [language_model, dependency_models]))
            self.identifiers.append((
                                    'Dependency Probability Feature (Language Model: ' + language_model + ') (Models: ' + dependency_models + ')',
                                    orientation))

    def addBinaryAllDependencyFeature(self, dep_counts_file, stanford_parser,
                                      dependency_models, java_path,
                                      orientation):
        """
        ...
        if orientation not in ['Complexity', 'Simplicity']:
            ...
            if dependency_models not in self.resources:
                parser = StanfordParser(path_to_jar=stanford_parser,
                                        path_to_models_jar=dependency_models)
                self.resources[dependency_models] = parser
            self.features.append((self.binaryAllDependencyFeature,
                                  [dep_counts_file, dependency_models]))
            self.identifiers.append((
                                    'Binary All Dependency Feature (Dependency Link Counts File: ' + dep_counts_file + ') (Models: ' + dependency_models + ')',
                                    orientation))

    def addAllDependencyFrequencyFeature(self, dep_counts_file, stanford_parser,
                                         dependency_models, java_path,
                                         orientation):
        """
        ...
        if orientation not in ['Complexity', 'Simplicity']:
            ...
            if dependency_models not in self.resources:
                parser = StanfordParser(path_to_jar=stanford_parser,
                                        path_to_models_jar=dependency_models)
                self.resources[dependency_models] = parser
            self.features.append((self.allDependencyFrequencyFeature,
                                  [dep_counts_file, dependency_models]))
            self.identifiers.append((
                                    'All Dependency Frequency Feature (Dependency Link Counts File: ' + dep_counts_file + ') (Models: ' + dependency_models + ')',
                                    orientation))

    def addWordVectorContextSimilarityFeature(self, model, pos_model,
                                              stanford_tagger, java_path,
                                              orientation):
        """
        ...
        if orientation not in ['Complexity', 'Simplicity']:
            ...
            if pos_model not in self.resources:
                ...
                self.resources[pos_model] = tagger
            self.features.append(
                (self.wordVectorContextSimilarityFeature, [model, pos_model]))
            self.identifiers.append((
                                    'Word Vector Context Similarity (Model: ' + model + ') (POS Model: ' + pos_model + ')',
                                    orientation))

    def addTaggedWordVectorContextSimilarityFeature(self, model, pos_model,
                                                    stanford_tagger, java_path,
                                                    pos_type, orientation):
        """
        ...
        if orientation not in ['Complexity', 'Simplicity']:
            ...
            if pos_model not in self.resources:
                tagger = StanfordPOSTagger(pos_model, stanford_tagger)
                self.resources[pos_model] = tagger
            self.features.append((self.taggedWordVectorContextSimilarityFeature,
                                  [model, pos_model, pos_type]))
            self.identifiers.append((
                                    'Tagged Word Vector Context Similarity (Model: ' + model + ') (POS Model: ' + pos_model + ') (POS Type: ' + pos_type + ')',
                                    orientation))

    def addNullLinkNominalFeature(self, stanford_parser, dependency_models,
                                  java_path, orientation):
        """
        ...
        if orientation not in ['Complexity', 'Simplicity']:
            ...
            self.features.append(
                (self.nullLinkNominalFeature, [dependency_models]))
            self.identifiers.append((
                                    'Null Link Nominal Feature (Models: ' + dependency_models + ')',
                                    orientation))
    ...
    def addBackoffBehaviorNominalFeature(self, ngram_file, orientation):
        ...
        if orientation not in ['Complexity', 'Simplicity']:
            ...
            self.features.append(
                (self.backoffBehaviorNominalFeature, [ngram_file]))
            self.identifiers.append((
                                    'N-Gram Nominal Feature (N-Grams File: ' + ngram_file + ')',
                                    orientation))
    ...
    def addImageSearchCountFeature(self, key, orientation):
        ...
        if orientation not in ['Complexity', 'Simplicity']:
            ...
            self.identifiers.append(
                ('Image Search Count Feature (Key: ' + key + ')', orientation))
    ...
    def addNgramNominalFeature(self, leftw, rightw):
        ...
        self.identifiers.append(('N-Gram Nominal Feature [' + str(
            leftw) + ', ' + str(rightw) + ']', 'Not Applicable'))

    def addCandidatePOSNominalFeature(self, pos_model, stanford_tagger,
                                      java_path, pos_type):
        """
        ...
        self.features.append(
            (self.candidatePOSNominalFeature, [pos_model, pos_type]))
        self.identifiers.append((
                                'Candidate POS Nominal Feature (POS Model: ' + pos_model + ') (POS Type: ' + pos_type + ')',
                                'Not Applicable'))

    def addPOSNgramNominalFeature(self, leftw, rightw, pos_model,
                                  stanford_tagger, java_path, pos_type):
        """
        ...
        self.features.append(
            (self.POSNgramNominalFeature, [leftw, rightw, pos_model, pos_type]))
        self.identifiers.append(('POS N-gram Nominal Feature [' + str(
            leftw) + ', ' + str(
            rightw) + '] (POS Model: ' + pos_model + ') (POS Type: ' + pos_type + ')',
                                 'Not Applicable'))

    def addPOSNgramWithCandidateNominalFeature(self, leftw, rightw, pos_model,
                                               stanford_tagger, java_path,
                                               pos_type):
        """
        ...
        self.features.append((self.POSNgramWithCandidateNominalFeature,
                              [leftw, rightw, pos_model, pos_type]))
        self.identifiers.append((
                                'POS N-gram with Candidate Nominal Feature [' + str(
                                    leftw) + ', ' + str(
                                    rightw) + '] (POS Model: ' + pos_model + ') (POS Type: ' + pos_type + ')',
                                'Not Applicable'))
    ...
    def addAverageTokenProbabilityFeature(self, language_model, orientation):
        ...
        if orientation not in ['Complexity', 'Simplicity']:
            ...
            if language_model not in self.resources:
                ...
                self.resources[language_model] = model
            self.features.append(
                (self.averageTokenProbabilityFeature, [language_model]))
            self.identifiers.append((
                                    'Average Token Probability (LM: ' + language_model + ')',
                                    orientation))
    ...
    def addMaximumTokenProbabilityFeature(self, language_model, orientation):
        ...
        if orientation not in ['Complexity', 'Simplicity']:
            ...
            if language_model not in self.resources:
                ...
                self.resources[language_model] = model
            self.features.append(
                (self.maximumTokenProbabilityFeature, [language_model]))
            self.identifiers.append((
                                    'Maximum Token Probability (LM: ' + language_model + ')',
                                    orientation))
    ...
    def addMinimumTokenProbabilityFeature(self, language_model, orientation):
        ...
        if orientation not in ['Complexity', 'Simplicity']:
            ...
            if language_model not in self.resources:
                ...
                self.resources[language_model] = model
            self.features.append(
                (self.minimumTokenProbabilityFeature, [language_model]))
            self.identifiers.append((
                                    'Minimum Token Probability (LM: ' + language_model + ')',
                                    orientation))
    ...
    def addMinimumWordVectorSimilarityFeature(self, model, orientation):
        ...
        if orientation not in ['Complexity', 'Simplicity']:
            ...
            if model not in self.resources:
                ...
                self.resources[model] = m
            self.features.append(
                (self.minimumWordVectorSimilarityFeature, [model]))
            self.identifiers.append((
                                    'Minimum Word Vector Similarity (Model: ' + model + ')',
                                    orientation))
    ...
    def addMaximumWordVectorSimilarityFeature(self, model, orientation):
        ...
        if orientation not in ['Complexity', 'Simplicity']:
            ...
            if model not in self.resources:
                ...
                self.resources[model] = m
            self.features.append(
                (self.maximumWordVectorSimilarityFeature, [model]))
            self.identifiers.append((
                                    'Maximum Word Vector Similarity (Model: ' + model + ')',
                                    orientation))
    ...
    def addAverageWordVectorSimilarityFeature(self, model, orientation):
        ...
        if orientation not in ['Complexity', 'Simplicity']:
            ...
            if model not in self.resources:
                ...
                self.resources[model] = m
            self.features.append(
                (self.averageWordVectorSimilarityFeature, [model]))
            self.identifiers.append((
                                    'Average Word Vector Similarity (Model: ' + model + ')',
                                    orientation))
