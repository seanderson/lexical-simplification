...
FEATURE_DIR = paths.CWI + "features_cwi/"
...
class CustomFeatureEstimator:
    ...
    def load_features(self):
        ...
        for feature in self.features:
			...
			print("feature ", feature,numpy.load(FEATURE_DIR + feature["name"] + ".npy") )
    ...
    def fill_dictionary_from_n_grams_SEW(self, dictionary, result, n):
        ...
        with open(N_GRAM_DIRECTORY + "SEW/SEW-1gm.total") as fd:
            (garbage,num_total) = fd.readline().split('=')
    ...
    for i in range(len(lines)):
        if len(lines[i][4].split(' ')) > 1:
            ...
            lines[i] = {'phrase': [lines[i][4]], 'words': [], 'hit': lines[i][0],
                        ...
                        'score': int(lines[i][9]), 'prob': float(lines[i][10]),
