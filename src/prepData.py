...
def build_lexicon(outfile="lex.pbz2",vocabsize=5000,art_subset=""):
    ...
    # Get the most common words and build index_to_word and word_to_index vectors
    vocab = sorted(word_freq.items(), key=lambda x: (x[1], x[0]), reverse=True)
    # Add words, in order of frequency
    vecs = { }
    i = 0
    while i < len(vocab) and len(vecs) < vocabsize:
        w = vocab[i][0]
        vecs[w] = np.zeros( vocabsize )
        vecs[w][i] = 1
        i += 1
...
def main():
    ...
    if buildType == "Debug":
        ...
        trainSubset = 'data/trainArticles.txt'
        testSubset = 'data/testArticles.txt'
        w2vFile = 'data/LexiconOneHot'+lexsize+'.pbz2'
        trainFile = 'data/train/'+buildType+'Train.pbz2'
        testFile = 'data/test/'+buildType+'Test.pbz2'
        validFile = 'data/train/'+buildType+'Valid.pbz2'
        idxFile = 'data/test/'+buildType+'Test.idx'
        ...
        sys.exit(1)
    build_lexicon(w2vFile,int(lexsize),trainSubset)
    #createTrainData(w2vFile,trainFile,trainSubset)
    #createTestData(trainFile,testFile,idxFile,testSubset)
    #createValidationData(trainFile,validFile,idxValidFile,testSubset) # validation set
